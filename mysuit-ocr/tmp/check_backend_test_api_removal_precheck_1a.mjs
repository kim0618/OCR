...
const REPORT = join(ROOT, "tmp", "backend_test_1a_test_api_removal_precheck.md");
...
const allowedNewOrModified = new Set([
  ...
  "../ocr-server/logs/codex_BACKEND_TEST_1A_TEST_API_REMOVAL_PRECHECK_NO_PROD_MODIFY.out.log",
  "../ocr-server/logs/codex_BACKEND_TEST_1A_TEST_API_REMOVAL_PRECHECK_NO_PROD_MODIFY.err.log",
  "../ocr-server/logs/codex_BACKEND_TEST_1B_TEST_IMAGES_API_REMOVE_PRECHECK.out.log",
  "../ocr-server/logs/codex_BACKEND_TEST_1B_TEST_IMAGES_API_REMOVE_PRECHECK.err.log",
...
check("report exists", existsSync(REPORT), relative(ROOT, REPORT));
const reportText = existsSync(REPORT) ? readFileSync(REPORT, "utf8") : "";
for (const section of requiredSections) {
  check(`${section} section exists`, reportText.includes(section));
}
...
const testImagesExists = existsSync(join(ROOT, "src", "app", "api", "test-images", "route.ts"));
const oneCReportExists = existsSync(join(ROOT, "tmp", "backend_test_1c_test_images_api_remove.md"));
check("test-images route exists or 1C removal report exists", testImagesExists || oneCReportExists);
for (const route of ["ocr-cache", "autofill-cache", "ground-truth"]) {
  check(`protected candidate route exists: ${route}`, existsSync(join(ROOT, "src", "app", "api", route, "route.ts")));
}
...
const aliasHits = [];
for (const file of walk(join(ROOT, "src")).filter((p) => /\.(ts|tsx|js|jsx)$/.test(p))) {
  const text = readFileSync(file, "utf8");
  if (/from\s+["']@\/lib|import\(\s*["']@\/lib/.test(text)) {
    aliasHits.push(relative(ROOT, file).replaceAll("\\", "/"));
  }
}
check("@/lib import 0 maintained", aliasHits.length === 0, aliasHits.join(", "));
...
if (failCount === 0) {
  console.log("[BACKEND_TEST_API_REMOVAL_PRECHECK_1A] PASS");
  process.exit(0);
}
console.log("[BACKEND_TEST_API_REMOVAL_PRECHECK_1A] FAIL");
process.exit(1);
