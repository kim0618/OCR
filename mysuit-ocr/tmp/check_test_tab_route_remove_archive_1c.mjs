...
const REPORT = join(ROOT, "tmp", "test_tab_1c_route_remove_archive.md");
...
check("backup route page exists", existsSync(join(BACKUP_DIR, "src", "app", "test", "page.tsx")));
...
for (const route of ["test-images", "ocr-cache", "autofill-cache", "ground-truth"]) {
  check(`API route ${route} exists`, existsSync(join(ROOT, "src", "app", "api", route, "route.ts")));
}

const srcFiles = walk(join(ROOT, "src")).filter((p) => /\.(ts|tsx|js|jsx)$/.test(p));
const importAliasHits = [];
const importBareHits = [];
const workspaceHits = [];
const libAliasHits = [];
...
for (const file of srcFiles) {
  ...
  if (/from\s+["']@\/components\/test|import\(\s*["']@\/components\/test/.test(text)) importAliasHits.push(fileRel);
  if (/from\s+["'][^"']*components\/test|import\(\s*["'][^"']*components\/test/.test(text)) importBareHits.push(fileRel);
...
check("src residual import @/components/test 0", importAliasHits.length === 0, importAliasHits.join(", "));
check("src residual import components/test 0", importBareHits.length === 0, importBareHits.join(", "));
...
const sidebarText = readTextIfExists(sidebarPath);
const defaultItemsMatch = sidebarText.match(/const\s+DEFAULT_ITEMS[\s\S]*?\];/);
const defaultItemsText = defaultItemsMatch?.[0] ?? "";
check("layout/sidebar/menu /test exposure 0", !/href:\s*["']\/test["']|label:\s*["']Test["']/.test(defaultItemsText));
...
check("only expected production removal plus allowed artifacts", unexpected.length === 0, unexpected.slice(0, 8).join("; "));

const failCount = checks.filter((c) => !c.ok).length;
console.log(`FAIL count: ${failCount}`);
if (failCount === 0) {
  console.log("[TEST_TAB_ROUTE_REMOVE_ARCHIVE_1C] PASS");
  process.exit(0);
}
console.log("[TEST_TAB_ROUTE_REMOVE_ARCHIVE_1C] FAIL");
process.exit(1);
