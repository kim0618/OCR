...
const REPORT = join(ROOT, "tmp", "backend_test_1b_test_images_api_remove_precheck.md");
const TEST_IMAGES_ROUTE = join(ROOT, "src", "app", "api", "test-images", "route.ts");
...
check("src/app/api/test-images/route.ts exists or 1C removal report exists", existsSync(TEST_IMAGES_ROUTE) || oneCReportExists);
...
for (const file of srcFiles) {
  if (file === TEST_IMAGES_ROUTE) continue;
  const text = readFileSync(file, "utf8");
  if (/\/api\/test-images|test-images/.test(text)) callerHits.push(rel(file));
  if (/from\s+["']@\/lib|import\(\s*["']@\/lib/.test(text)) aliasHits.push(rel(file));
}
check("src operating caller /api/test-images 0", callerHits.length === 0, callerHits.join(", "));
...
if (failCount === 0) {
  console.log("[BACKEND_TEST_IMAGES_API_REMOVE_PRECHECK_1B] PASS");
  process.exit(0);
}
console.log("[BACKEND_TEST_IMAGES_API_REMOVE_PRECHECK_1B] FAIL");
process.exit(1);
