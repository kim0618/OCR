# FRONTEND_AUTORESTORE_TO_RESTORE_PRECHECK_20260522
...
- src/components/restore/** 상태: absent
...
- currentPath: src/components/autorestore/AutoRestoreWorkspace.tsx
- lineCount: 434
- imports: [{"symbols": "React, { useEffect, useState }", "source": "react", "typeOnly": false}, {"symbols": "{ useUi }", "source": "../layout/AppProviders", "typeOnly": false}, {"symbols": "{ type RestoreProfile, type RestoreProfileFields, PROFILE_FIELD_LABELS, readRestoreProfiles, deleteRestoreProfile, sortRestoreProfilesByUpdatedAt, }", "source": "@/lib/restoreProfileStore", "typeOnly": false}]
- exports: default function AutoRestoreWorkspace
- mainResponsibility: Restore/autofill profile 관리 workspace. 저장된 restore profile 목록 조회, 상세 표시, 삭제 UI를 제공한다.
- restore profile 관리 여부: True
- autofill profile 관리 여부: True
- restoreProfileStore 의존: True
- profiles 의존: False
- autofillEngine 의존: False
- historyStore/common storage 직접 의존: False
- AppProviders/useUi 의존: True
- RequireLogin 직접 의존: False
- localStorage/IndexedDB 직접 사용: False
- React state/effect 사용: True
- moveRisk: MEDIUM

## 6. route/importedBy 분석
- src/app/autorestore/page.tsx:5 [app, static] import AutoRestoreWorkspace from "../../components/autorestore/AutoRestoreWorkspace";

- route file: src/app/autorestore/page.tsx
- route URL: /autorestore 유지 권장
- 현재 import: ../../components/autorestore/AutoRestoreWorkspace
- 추천 import: ../../components/restore/AutoRestoreWorkspace
- dynamic import 여부: False
- route policy 변경 필요: False
...
## 8. dependency 영향
- restoreProfileStore import: True
- profiles import: False
- autofillEngine import: False
- common/storage historyStore direct import: False
- useUi import: True
- RequireLogin 관계: Route page wraps workspace with RequireLogin; workspace itself does not import RequireLogin.
- 상대 import 영향: ../layout/AppProviders becomes ../layout/AppProviders if moved to components/restore (same depth); no change needed for this relative import.
- 순환 의존 위험: LOW
...
- 간접 영향: restoreProfileStore/profiles/autofillEngine remain unmoved in recommended step
