# FRONTEND_IMAGE_STORE_COMMON_STORAGE_MOVE_PRECHECK_20260522
...
## 5. imageStore 역할 요약
- currentPath: src/lib/imageStore.ts
- lineCount: 117
- imports: []
- exports: 
...
- IndexedDB 사용: True
- localStorage 사용: True
- browser API 사용: True
- Blob/File/base64/objectURL 사용: True
- backend/API 사용: False
- React 의존: False
- components/* 의존: False
- common/storage 적합성: COMMON_STORAGE_READY_WITH_IMPORT_ONLY
- moveRisk: MEDIUM

## 6. importedBy 분석
- src/lib/historyStore.ts:5 [lib, runtime] import { saveImage as idbSaveImage, getImage as idbGetImage, deleteImagesFor as idbDeleteImagesFor } from "./imageStore";
- src/components/runocr/RunOcrWorkspace.tsx:38 [runocr, runtime] import { getTemplateImage } from "@/lib/imageStore";
- src/app/template/page.tsx:7 [template, runtime] import { getTemplateImage } from "@/lib/imageStore";
- src/components/template/ui/TemplateAnnotator.tsx:8 [template, runtime] import { saveTemplateImage, getTemplateImage, deleteTemplateImage } from "@/lib/imageStore";
...
## 7. dependency 영향
- historyStore -> imageStore: True
- imageStore -> historyStore: False
- imageStore -> components/*: False
- TemplateAnnotator 직접 import: True
- Template route 직접 import: True
- RunOCR 직접 import: True
- DetailHistoryView 직접 import: False
- HistoryWorkspace 직접 import: False
- 순환 의존 위험: LOW: imageStore has no imports and does not depend on historyStore/components.
- historyStore 이동 도움: After imageStore moves to common/storage, historyStore can later move to common/storage and import sibling ./imageStore.
