# T-9-extend Template/RunOCR E2E invoice_statement 확장 검증 결과

## 1. 생성 파일
- Script: `D:\Free_Vue\OCR\ocr-server\scripts\verify_invoice_statement_template_runocr_e2e_t9_extend.py`
- Markdown report: `D:\Free_Vue\OCR\mysuit-ocr\public\data\testsets\invoice_statement\reports\T9_extend_template_runocr_e2e_invoice_statement_20260514.md`
- JSON report: `D:\Free_Vue\OCR\mysuit-ocr\public\data\testsets\invoice_statement\reports\T9_extend_template_runocr_e2e_invoice_statement_20260514.json`
...
| 문제 | 원인 추정 | 후속 |
|---|---|---|
| 5.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
| 2.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
| 7.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
| 6.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
| 3.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
| 4.pdf: saved annotation missing | templates.json에 해당 샘플 파일명과 연결된 table region template 없음 | UI에서 table region/column guide 저장 후 T-9-extend 재실행 |
...
## 10. 검증 결과
- script py_compile: passed (`.venv\Scripts\python.exe -m py_compile`)
...
## 11. 다음 작업 판단
- 저장 annotation 부족 → UI에서 2~7.pdf template annotation 저장 후 재검증
