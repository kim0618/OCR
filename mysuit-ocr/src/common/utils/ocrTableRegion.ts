import type { Rect, TableRowOverride } from "../types/ocr";
import { clampRectToArea } from "./ocrCanvasOps";
...
/**
 * TPL-12A: Minimum row height (in the same coordinate space as `Rect`, i.e.
 * image-pixel space at the template's canonical resolution). Aligned with the
 * `minSize = 4` clamp used by OcrCanvasPane drag handlers, and ensures we
 * never emit a degenerate row from a malformed override.
 */
...
/**
 * TPL-12A: Apply sparse per-row overrides to a base rows array and re-cascade
 * subsequent rows' y so each row sits directly below its predecessor. Clamps
 * the result inside `area` and trims trailing rows that no longer fit.
 *
 * Pure: never mutates any of `baseRows`, `rowOverrides`, `area`, or any of
 * their nested objects. Always returns a fresh array with fresh Rect objects.
 *
 * Policy:
 *  - rowOverrides undefined/empty → clone of baseRows (same shape).
 *  - Each override is keyed by `rowIndex` (0-based). Invalid overrides are
 *    ignored:
 *      * rowIndex not a finite integer
 *      * rowIndex < 0 OR rowIndex >= baseRows.length
 *      * height non-finite OR height < MIN_ROW_HEIGHT (after numeric guard)
 *      * y non-finite (height-only override still allowed)
 *    A partially-valid override (e.g. only valid `height`) still applies the
 *    valid field; the other is ignored.
 *  - When an override's `y` is present and valid, that row is placed at the
 *    given y AND subsequent rows cascade from `y + height`.
 *  - When only `height` is overridden, the row keeps its base y and rows after
 *    it cascade by the new height.
 *  - `locked` is read but ignored at materialization time (TPL-12C UI scope).
 *  - After cascading, the entire result is clamped to `area`. Any row that
 *    would extend past `area.y + area.height` is either height-clamped down to
 *    fit (if the remaining space is ≥ MIN_ROW_HEIGHT) or trimmed. Rows whose
 *    starting y is already past the area bottom are dropped.
 */
export function materializeTableRowsWithOverrides(
  ...
  // Index overrides by rowIndex with last-wins on duplicates. Validation
  // happens at consumption — we keep raw shape here for inspection.
  ...
  // Cascade through base rows, applying valid override fields. We work in a
  // fresh array of cloned Rects so callers never see aliased objects.
  const out: Rect[] = [];
  ...
  for (let i = 0; i < baseRows.length; i++) {
    ...
    // Resolve target height: override > base.
    let height = base.height;
    if (ov && isFiniteNumber(ov.height) && ov.height >= MIN_ROW_HEIGHT) {
      height = ov.height;
    }

    // Resolve target y: override > cascade > base.
    let y: number;
  ...
  // Bounds clamp + trailing trim. We clamp x/width to area first via the
  // existing helper, then enforce y in [area.y, area.y + area.height] and trim
  // rows that no longer fit.
  const result: Rect[] = [];
  const areaTop = area.y;
  const areaBottom = area.y + area.height;
  for (const r of out) {
    // x/width clamp via the canvas helper (also re-clamps y/height as side
    // effect, so we re-apply y/height explicitly afterwards).
    const xyClamped = clampRectToArea(cloneRect(r), area);

    // Drop rows whose start sits at/below the area bottom.
    if (r.y >= areaBottom) continue;

    let y = Math.max(r.y, areaTop);
    let height = r.height;
