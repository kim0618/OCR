...
import {
  ...
  parseIndex,
  uid,
} from "../utils/ocrCanvasOps";
import {
  MIN_ROW_HEIGHT,
  buildTableRows,
  materializeTableRowsWithOverrides,
  normalizeColGuides,
} from "../utils/ocrTableRegion";
...
type Props = {
  ...
  /**
   * TPL-13A: 컬럼 정의 row focus/click 시 활성화되는 column highlight target.
   * 해당 table region에 colGuides 기반으로 계산된 column interval overlay를
   * 렌더링한다. pointerEvents none이므로 다른 mode와 충돌하지 않는다.
   */
...
export default function OcrCanvasPane(props: Props) {
  ...
  // ===== TPL-12C / TPL-12G: row boundary drag (행 개별 조정) =====
  // DragKind (types/ocr.ts)에 추가 variant를 만들 수 없는 phase 제약 때문에
  // 별도 ref/state로 보관한다. 일반 DragKind와 동시에 활성화되지 않도록
  // pointerDown 시점에 setDragBoth(null)을 함께 호출한다.
  //
  // TPL-12G: drag policy를 height-only cascade에서 LOCAL BOUNDARY ADJUST로
  // 바꿈. boundary i/i+1 드래그는 row i / row i+1 두 행만 갱신하고, row i+2
  // 이후 rows는 원래 y를 유지한다. drag-start 시 row i의 top y와 row i+1의
  // bottom y(또는 area bottom)를 캡처해 boundary 위치 계산의 anchor로 사용.
  type RowBoundaryDrag = {
    tableId: string;
    /** 0-based index of the row whose BOTTOM boundary is being dragged. */
    rowIndex: number;
    startY: number;
    /** TPL-12G: row[i].y at drag start — upper anchor for boundary clamp. */
    rowTopY: number;
    /** TPL-12G: row[i+1].y + row[i+1].height at drag start, or area bottom
     *  when row[i] is the last row — lower anchor for boundary clamp. */
    nextBottomY: number;
    /** TPL-12G: true iff a next row exists; when false, the drag only
     *  changes row[i].height (no row[i+1] override is emitted). */
    hasNextRow: boolean;
  };
  const [rowBoundaryDrag, setRowBoundaryDrag] = useState<RowBoundaryDrag | null>(null);
      ...
      setRegions((prev) =>
        prev.map((r) => {
          ...
          if (r.fieldType === "table" && r.table) {
            ...
            // TPL-12G: rowOverrides 중 absolute y가 들어 있는 항목은 region
            // 이동량만큼 함께 shift해야 한다 (TPL-12C local-adjust drag가
            // row[i+1].y에 absolute y를 저장하므로). height-only / locked
            // 항목은 그대로 유지.
            const shiftedRowOverrides = Array.isArray(r.table.rowOverrides)
              ? (r.table.rowOverrides as TableRowOverride[]).map((ov) =>
            ...
            return {
              ...
              table: {
                ...
                rows: Array.isArray(r.table.rows)
                  ...
                  : undefined,
                rowOverrides: shiftedRowOverrides,
              },
            };
          }
          return { ...r, x: nx, y: ny };
  ...
  // TPL-12G: upsert helper for sparse rowOverrides.
  //   - `patch` 필드 중 undefined는 기존 값을 덮지 않음 (partial merge).
  //   - locked 같은 기존 부가 메타는 보존.
  //   - rowIndex 오름차순 정렬은 helper가 다시 sort하지 않으므로 호출부가
  //     필요 시 따로 처리한다 (materializeTableRowsWithOverrides는 indexed
  //     Map 으로 lookup하므로 정렬에 의존하지 않음).
  ...
  function applyRowBoundaryDragFrame(pt: { x: number; y: number }) {
    ...
    // TPL-12G: local boundary adjust.
    //   boundaryY ∈ [rowTopY + MIN, nextBottomY - MIN]   (hasNextRow)
    //            ∈ [rowTopY + MIN, areaBottom]           (last row)
    //
    //   After:
    //     row[i].height       = boundaryY - rowTopY
    //     row[i+1].y          = boundaryY                 (only if hasNextRow)
    //     row[i+1].height     = nextBottomY - boundaryY   (only if hasNextRow)
    //     row[i+2..].y/height = preserved by helper cascade
    //       since row[i+1].y + row[i+1].height = nextBottomY = original
    //       row[i+2].y, so the cascade naturally lands on the old position.
    ...
    // baseRows for materialization: prefer current rows (reflect rowTemplate
    // + already-applied overrides); fall back to buildTableRows.
  ...
  function onPointerUp() {
    ...
    // TPL-12C: row boundary drag has its own ref; clear it on pointer up
    // regardless of the standard DragKind state.
    if (rowBoundaryDragRef.current) {
        ...
        <>
          ...
          >
            ...
            {visibleRegions.map((r) => {
              ...
              // TPL-12C: rows we DISPLAY on canvas are materialized with the
              // current rowOverrides so the user always sees the same rows the
              // export payload will save. baseRows stays the underlying
              // (possibly equal-height) array.
                ...
                >
                  ...
                  {isSel && r.fieldType === "table" && (
                    <>
                      ...
                      {/* TPL-12C/12G: 행 경계 핸들 — rowAdjust 모드에서만 표시.
                        ...
                        tableRows.slice(0, 80).map((rr, idx) => (
                          <div
                            ...
                            onPointerDown={(e) => {
                              ...
                              e.stopPropagation();
                              setSelectedId(r.id);
                              // mutually exclusive — clear any other drag in flight.
                              setDragBoth(null);
                              const p = getImagePoint(e.clientX, e.clientY);
                              if (!p) return;
                              // TPL-12G: capture row[i] top y and row[i+1]
                              // bottom y at drag start so the boundary clamp
                              // & cascade math doesn't drift between frames.
                  ...
                  {/* TPL-13A: column highlight overlay — TemplateRightPanel의
                      컬럼 정의 row가 focus/click되었을 때 해당 column interval을
                      반투명 노랑 박스로 강조한다. pointerEvents none이므로
                      다른 모드(drawRowTemplate / colGuide / rowAdjust / resize)와
                      충돌하지 않는다. region 선택 여부와 무관하게 표시. */}
                  {r.fieldType === "table"
                    && selectedTableColumnTarget
                    && selectedTableColumnTarget.regionId === r.id
                    && (() => {
                      const colHL_guides = normalizeColGuides(r.table?.colGuides);
                      const colHL_colCount = colHL_guides.length + 1;
                      const colHL_idx = selectedTableColumnTarget.columnIndex;
                      if (colHL_idx < 0 || colHL_idx >= colHL_colCount) return null;
                      const colHL_left =
                        colHL_idx === 0 ? 0 : colHL_guides[colHL_idx - 1];
                      const colHL_right =
                        colHL_idx === colHL_guides.length ? 1 : colHL_guides[colHL_idx];
                      const colHL_widthRatio = Math.max(0, colHL_right - colHL_left);
                      return (
                        <div
                          data-role="column-highlight-overlay"
                          style={{
                            position: "absolute",
                            left: `${colHL_left * 100}%`,
                            top: 0,
                            width: `${colHL_widthRatio * 100}%`,
                            height: "100%",
                            background: "rgba(250,204,21,0.18)",
                            border: "1px dashed rgba(202,138,4,0.95)",
                            boxSizing: "border-box",
                            pointerEvents: "none",
                            zIndex: 28,
                            borderRadius: 4,
                          }}
                          title={`컬럼 ${colHL_idx + 1}`}
                        >
                          <span
                            style={{
                              position: "absolute",
