/**
 * RunOCR 탭의 최상위 Workspace 컴포넌트.
 *
 * 역할:
 * - 파일 업로드, 템플릿/모델 선택, OCR 실행, 결과 표시 흐름을 조립한다.
 * - 자동복원(autofill) 후보 적용과 히스토리(history) 저장 등 OCR 실행 후처리를
 *   오케스트레이션한다.
 * - viewer / resultPanel / scanOverlay / hiddenFileInput 노드를 만들어
 *   RunOcrResultLayout 에 전달한다.
 *
 * 책임 경계:
 * - FormData 구성        → utils/buildOcrFormData.ts
 * - OCR API 호출         → utils/runOcrRequest.ts (endpoint / fetch / ok / json)
 * - raw 응답 매핑        → utils/mapOcrResponse.ts (buildRunOcrResult)
 * - 결과 화면 layout     → ui/RunOcrResultLayout.tsx (node composition)
 *
 * 주의:
 * - autofill / history / restore / setOcrResult 흐름은 아직 이 Workspace 가
 *   오케스트레이션한다. 분리는 후속 phase 대상.
 * - normalizeAutofillFieldKey 는 mapOcrResponse 의 옵션으로 주입한다
 *   (mapOcrResponse 가 autofillEngine 을 직접 import 하지 않도록).
 */
"use client";
...
type RunOcrWorkspaceProps = {
  variant?: RunOcrWorkspaceVariant;
};
...
/**
 * RunOCR Workspace 컴포넌트.
 *
 * variant 는 "upload" (기본) / "runocr" 두 가지. RunOCR 탭에서 사용될 때는
 * `variant="runocr"` 로 진입하며, 모델 선택 / 자동복원 적용 / 히스토리 기록
 * 같은 RunOCR 전용 흐름이 활성화된다.
 *
 * 파일 단독 미리보기 흐름과 결과 화면(`<RunOcrResultLayout>`) 두 branch 를
 * `ocrResult && selectedFile` 조건으로 분기 렌더한다.
 */
export default function RunOcrWorkspace({ variant = "upload" }: RunOcrWorkspaceProps) {
  ...
  // buildRunOcrResult was extracted to ./utils/mapOcrResponse in
  // FRONTEND-STRUCTURE-2C. Inject the field-key normalizer at call site so
  // the helper stays independent of autofillEngine.
  ...
  /**
   * 사용자가 선택한 파일과 템플릿/모델 정보를 기반으로 OCR 을 실행한다.
   *
   * 처리 흐름:
   * 1. runOcrRequest 로 `/ocr/extract` API 를 호출한다 (FormData 구성과
   *    fetch / ok / json 처리는 모두 utils 에 위임).
   * 2. buildRunOcrResult 로 raw 응답을 화면용 OcrResult 로 변환한다.
   * 3. 사업자번호 기반 자동복원 후보를 수집/적용해 결과 필드를 보강한다.
   * 4. 성공/실패에 따라 히스토리(appendHistoryRun / updateHistoryRun) 를
   *    기록하고 결과 상태(setOcrResult / setCurrentJobId 등) 를 반영한다.
   *
   * 주의:
   * - FormData key 정책 / fetch 호출 / response shape 변경은 여기서 하지 않는다.
   * - 에러 메시지 `"OCR 처리 중 오류가 발생했습니다."` 와 fail 히스토리 기록은
   *   기존 catch 흐름과 묶여 있으므로 임의 변경 금지.
   */
  ...
  /**
   * 결과 패널에서 사용자가 편집한 output 필드를 히스토리에 머지 저장한다.
   *
   * initialOutputFields(success 시점 스냅샷)에서 immutable 메타 (no/en/ko/
   * original/confidence/source/applied/autofillAction/suggestions) 를 보존하고
   * 사용자 편집 값만 `modified` 로 덮어쓴다. `currentJobId` 또는 스냅샷이
   * 없으면 (fail/no-op 케이스) 저장을 건너뛴다.
   */
  ...
  /**
   * 결과 화면의 X 버튼 핸들러: RunOCR 진입 직후(파일 미선택) 상태로 되돌린다.
   * 결과/전처리 이미지/현재 job 메타/선택 인덱스/탭/스캔 상태 등을 초기화한다.
   */
  ...
  if (ocrResult && selectedFile) {
    const resultViewerNode = ocrDisplayUrl && resultTab === "custom" && canvasLoaded ? (
      <OcrCanvasPane
        imgRef={canvasImgRef}
        loaded={canvasLoaded}
    ...
    const resultScanOverlayNode = isOcrRunning
      ? <div className="uw-scan-overlay"><div className="uw-scan-line" /></div>
      : null;
    ...
    const resultHiddenFileInputNode = (
      <input
        ref={fileInputRef}
        type="file"
