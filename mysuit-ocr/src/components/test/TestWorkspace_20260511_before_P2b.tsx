...
// ── amountProfile별 표시 금액/summary 필드 (P-2) ──
...
function getAmountFieldKeysForProfile(amountProfile?: string): string[] {
  if (!amountProfile) return ["supplyAmount", "taxAmount", "totalAmount"];
  return AMOUNT_PROFILE_AMOUNT_FIELDS[amountProfile] ?? ["supplyAmount", "taxAmount", "totalAmount"];
}

// DocumentDetailPanel에서 summary field (DOCUMENT_COLUMNS 외부) 카드 메타 생성
function buildSummaryFieldMeta(keys: string[]): { key: string; label: string; shortLabel: string; required: boolean }[] {
  return keys
    .filter(k => !STANDARD_AMOUNT_KEYS_IN_META.has(k))
    .map(k => ({
      key: k,
      label: DOCUMENT_FIELD_LABELS[k] ?? k,
      shortLabel: DOCUMENT_FIELD_SHORT[k] ?? DOCUMENT_FIELD_LABELS[k] ?? k,
      required: false,
    }));
}
...
function DocumentDetailPanel({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {(() => {
        // amountProfile 기반 visible fields 계산 (P-2)
        const applicableKeys = getAmountFieldKeysForProfile(invoiceProfile?.amountProfile);
        // DOCUMENT_FIELD_META에서 standard amount keys 필터링 (applicable만 통과)
        const visibleMeta = DOCUMENT_FIELD_META.filter(({ key }) =>
          STANDARD_AMOUNT_KEYS_IN_META.has(key) ? applicableKeys.includes(key) : true
        );
        // DOCUMENT_COLUMNS 밖의 summaryField 카드 메타
        const summaryMeta = buildSummaryFieldMeta(applicableKeys);
        return [...visibleMeta, ...summaryMeta].map(({ key, label, required }) => (
          <DocumentFieldCard
            key={key}
            fieldKey={key}
            label={label}
