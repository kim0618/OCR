...
function getPartyMasterCandidateForField(
  ...
  const bizNumber = normalizePartyMasterBizNumber(documentFields?.[bizField]);
  if (!bizNumber) return null;

  const value = partyMaster[bizNumber]?.[masterField];
          ...
          <>
            ...
            {[...visibleMeta, ...summaryMeta].map(({ key, label, required }) => (
              <DocumentFieldCard
                ...
                masterCandidate={getPartyMasterCandidateForField(key, documentFields, partyMaster, documentGt[key] ?? "")}
              />
            ))}
          </>
        );
