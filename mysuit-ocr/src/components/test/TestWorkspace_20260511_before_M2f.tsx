...
type PartyMasterPromotion = {
  ...
  selectedSource: "master" | "ocr" | "norm" | "existing";
  reason: string;
  conflict?: boolean;
};
...
// M-2e: 통일된 final status 계산 함수
// O   = OCR/NORM이 GT와 직접 일치
// △   = MASTER/GT_SIMILARITY/GT_ANCHOR_EMPTY 등 보정으로 채택된 값
// X   = 불일치 또는 미추출
// —   = GT 없음
function computeFieldFinalStatus(args: {
  ...
  // OCR/NORM 부분 일치 (GT_SIMILARITY 해당) → △
  ...
  // OCR 미추출 + GT 있음 (GT_ANCHOR_EMPTY 해당) → △
  if (!ocrValue && !normalizedValue) return "△";
  return "X";
}
...
export default function TestWorkspace() {
  ...
  const documentKpi = useMemo((): DocumentKpiSummary => {
    ...
    for (const img of documentBatchRows) {
      ...
      if (hasAnyGt) {
        gtImageCount++;
        // M-RESET: MASTER 제거 — GT vs OCR/NORM 직접 비교로 복구
                  ...
                  return (
                    <tr key={img} onClick={() => setSelected(img)}
                      ...
                      {DOCUMENT_FIELD_META.map((col) => {
                        ...
                        // M-RESET: GT vs OCR/NORM 직접 비교 복구 (MASTER 제거)
...
function getPartyMasterPromotion(args: {
  ...
  // M-RESET: Test 단계에서는 GT vs OCR/NORM이 유일 기준 — MASTER 승격 비활성화
  const { rawValue, normalizedValue } = args;
  const existingValue = normalizedValue || rawValue;
  return { promoted: false, selectedValue: existingValue, selectedSource: "existing", reason: "MASTER disabled in test stage" };
}
...
function DocumentFieldCard({
  ...
  const finalView = (() => {
    ...
    // M-2c: MASTER 승격이 결정된 경우 GT 여부·OCR 일치 여부에 관계없이 MASTER 우선 채택
