...
type PartyMasterPromotion = {
  ...
  selectedSource: "master" | "existing";
  reason: string;
  conflict?: boolean;
};
...
export default function TestWorkspace() {
  ...
  type DocumentKpiSummary = {
    ...
    masterSelectedCount: number;
    masterGtMatch: number;
    masterAssistedGtFieldMatch: number;
  ...
  const documentKpi = useMemo((): DocumentKpiSummary => {
    ...
    let gtImageCount = 0, gtFieldTotal = 0, gtFieldMatch = 0;
    let masterCandidateCount = 0, masterSelectedCount = 0, masterGtMatch = 0, masterAssistedGtFieldMatch = 0;
    ...
    for (const img of documentBatchRows) {
      ...
      if (hasAnyGt) {
        ...
        for (const k of allEvalFields) {
          ...
          const selectedWithMaster = getDocumentSelectedValueWithMaster({
            fieldKey: k,
            documentFields: docFields,
            normalization: getInvoiceNormalization(ocrEntry),
            partyMaster,
            gtValue: gtVal,
            invoiceProfile: invProfile,
          });
          if (selectedWithMaster.candidate) masterCandidateCount++;
          if (selectedWithMaster.source === "master") {
            masterSelectedCount++;
            if (gtVal && documentFieldMatches(gtVal, selectedWithMaster.value)) masterGtMatch++;
          }
          if (gtVal) {
            gtFieldTotal++;
            // party field이면 partyFieldGtMatch도 업데이트
            if (Object.prototype.hasOwnProperty.call(partyFieldGtMatch, k)) {
            ...
            if (documentFieldMatches(gtVal, selectedWithMaster.value) || gtVal === selectedWithMaster.value) {
              masterAssistedGtFieldMatch++;
            }
          }
        }
      }
    ...
    return {
      ...
      gtImageCount, gtFieldTotal, gtFieldMatch,
      masterCandidateCount, masterSelectedCount, masterGtMatch, masterAssistedGtFieldMatch,
      partyFieldGtMatch,
    };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 12 }}>
      <div style={styles.datasetBar}>
        ...
        <div style={{ width: "100%", display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <SourceLegend label="MASTER" color="#f97316" note="사업자번호 기반 party 자동채움 후보" />
      ...
      {batchRows.length > 0 && (
        <div style={styles.kpiWrapper}>
          ...
          >
            ...
            {documentKpi.masterCandidateCount > 0 && (
              <>
                <KpiChip label="MASTER 후보" value={String(documentKpi.masterCandidateCount)} tone="neutral" title="사업자번호 OCR/NORM 값을 key로 찾은 party master 후보 수" />
                <KpiChip label="MASTER 채택" value={String(documentKpi.masterSelectedCount)} tone={documentKpi.masterSelectedCount > 0 ? "amber" : "neutral"} title="party 고정 필드 6개 중 selectedValue가 MASTER로 승격된 수" />
                <KpiChip label="MASTER-GT" value={`${documentKpi.masterGtMatch}/${documentKpi.masterSelectedCount}`} tone={toneOf(documentKpi.masterGtMatch, documentKpi.masterSelectedCount)} title="MASTER selected 값과 GT가 일치한 수. 기존 OCR GT 일치율과 별도입니다." />
              </>
            )}
            {documentKpi.gtFieldTotal > 0 && documentKpi.masterSelectedCount > 0 && (
              <KpiChip
                label="MASTER 보조 후"
                value={pct(documentKpi.masterAssistedGtFieldMatch, documentKpi.gtFieldTotal)}
                sub={`${documentKpi.masterAssistedGtFieldMatch}/${documentKpi.gtFieldTotal} fields`}
                tone={toneOf(documentKpi.masterAssistedGtFieldMatch, documentKpi.gtFieldTotal)}
                title="OCR 자체 GT 일치율은 그대로 두고, MASTER selectedValue를 반영한 제품 결과 관점 일치율입니다."
              />
            )}
...
function getPartyMasterPromotion(args: {
  ...
  const { candidate, rawValue, normalizedValue, normalizationRecord, invoiceProfile } = args;
  const existingValue = normalizedValue || rawValue;
  if (!candidate) {
    return { promoted: false, selectedValue: existingValue, selectedSource: "existing", reason: "no master candidate" };
  }
  if (invoiceProfile?.partyProfile === "buyer_only" && candidate.role === "supplier") {
    return { promoted: false, selectedValue: existingValue, selectedSource: "existing", reason: "buyer_only supplier skip" };
  }
  if (invoiceProfile?.partyProfile === "buyer_rep_optional" && candidate.fieldKey === "buyerRepresentative") {
    return { promoted: false, selectedValue: existingValue, selectedSource: "existing", reason: "buyerRepresentative optional skip" };
  }
  if (!rawValue && !normalizedValue) {
    return { promoted: true, selectedValue: candidate.value, selectedSource: "master", reason: "OCR empty; biz master autofill" };
  }

  const rawCompact = compactPartyMasterValue(candidate.masterField, rawValue);
  const normCompact = compactPartyMasterValue(candidate.masterField, normalizedValue);
  const masterCompact = compactPartyMasterValue(candidate.masterField, candidate.value);
  const bestCompact = normCompact || rawCompact;
  const similarity = normalizedEditSimilarity(bestCompact, masterCompact);
  const containsMatch = Boolean(bestCompact && (bestCompact.includes(masterCompact) || masterCompact.includes(bestCompact)));
  const partialAddress = candidate.masterField === "address" && (
    containsMatch ||
    hasSharedAddressDigit(candidate.value, rawValue) ||
    hasSharedAddressDigit(candidate.value, normalizedValue) ||
    normalizationRecord?.addressSimilarityAnalysis?.appearsTruncated === true ||
    Boolean(normalizationRecord?.addressSimilarityAnalysis?.addressSimilarityStatus && normalizationRecord.addressSimilarityAnalysis.addressSimilarityStatus !== "complete")
  );

  if (containsMatch) {
    return { promoted: true, selectedValue: candidate.value, selectedSource: "master", reason: "OCR/NORM close to master" };
  }
  if (candidate.masterField !== "address" && similarity >= 0.58) {
    return { promoted: true, selectedValue: candidate.value, selectedSource: "master", reason: `OCR/NORM similar to master (${similarity.toFixed(2)})` };
  }
  if (partialAddress) {
    return { promoted: true, selectedValue: candidate.value, selectedSource: "master", reason: "address partial/tail missing; biz master autofill" };
  }
  if (candidate.masterField === "address" && similarity >= 0.45) {
    return { promoted: true, selectedValue: candidate.value, selectedSource: "master", reason: `address similar to master (${similarity.toFixed(2)})` };
  }
  ...
  return {
    ...
    reason: `master conflict/low similarity (${similarity.toFixed(2)})`,
    conflict: true,
  };
}
...
function getDocumentSelectedValueWithMaster(args: {
  ...
  gtValue: string;
  invoiceProfile?: InvoiceProfile;
}): { value: string; source: "master" | "ocr"; candidate: PartyMasterCandidate | null; promotion: PartyMasterPromotion } {
  ...
  if (promotion.promoted && candidate) return { value: candidate.value, source: "master", candidate, promotion };
...
function DocumentFieldCard({
  ...
  const finalView = (() => {
    if (hasOcr && masterPromotion.promoted && masterCandidate) {
      return { value: masterCandidate.value, source: "master" as const, reason: masterPromotion.reason };
    }
  ...
  const srcMeta: Record<typeof finalView.source, { label: string; color: string; bg: string; border: string }> = {
    ...
    master:           { label: "MASTER",           color: "#f97316", bg: "rgba(249,115,22,0.12)",  border: "rgba(249,115,22,0.38)" },
  ...
  return (
    <div style={{
      ...
      {masterCandidate && (
        <FieldSlot color="#f97316" label="MASTER">
          <div style={{ display: "flex", flexDirection: "column", gap: 5, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span style={{
                fontSize: 12,
                ...
                {masterCandidate.value}
              </span>
              <span title={masterCandidate.source} style={{
                fontSize: 9, fontWeight: 900, padding: "2px 7px", borderRadius: 999,
                color: "#fff", background: "#f97316", whiteSpace: "nowrap",
              }}>
                MASTER
              </span>
              {masterCandidate.matchesGt && (
                <span style={{
                  fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 999,
                  color: "#fed7aa", background: "rgba(249,115,22,0.12)",
                  border: "1px solid rgba(249,115,22,0.28)", whiteSpace: "nowrap",
                }}>
                  GT match
                </span>
              )}
              {masterPromotion.promoted ? (
                <span style={{ fontSize: 9, fontWeight: 900, padding: "2px 7px", borderRadius: 999, color: "#fff7ed", background: "rgba(249,115,22,0.26)", border: "1px solid rgba(249,115,22,0.4)", whiteSpace: "nowrap" }}>
                  selected
                </span>
              ) : masterPromotion.conflict ? (
                <span style={{ fontSize: 9, fontWeight: 900, padding: "2px 7px", borderRadius: 999, color: "#fecaca", background: "rgba(239,68,68,0.14)", border: "1px solid rgba(239,68,68,0.3)", whiteSpace: "nowrap" }}>
                  conflict
                </span>
              ) : (
                <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 999, color: "#fed7aa", background: "rgba(249,115,22,0.10)", border: "1px solid rgba(249,115,22,0.22)", whiteSpace: "nowrap" }}>
                  candidate
                </span>
              )}
            </div>
            <div style={{ fontSize: 10, color: "var(--muted)", lineHeight: 1.45, wordBreak: "break-word" }}>
              source: {masterCandidate.source} · biz: {masterCandidate.bizNumber} · field: {masterCandidate.masterField}
            </div>
          </div>
        </FieldSlot>
      )}
        ...
        <div style={{
          ...
          {!hasOcr ? (
            ...
            <>
              ...
              {finalView.source === "master" && masterCandidate && (
                <span title={`${masterCandidate.source} / ${masterCandidate.bizNumber}`} style={{ fontSize: 10, color: "#fed7aa", fontWeight: 800 }}>
                  {masterCandidate.source} · {masterCandidate.bizNumber}
                </span>
              )}
