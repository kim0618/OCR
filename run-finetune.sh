...
PY
    ...
    #  ★위 곡선은 <결정성 확보 이전>이라 설정 효과와 실행 변동이 섞여 있다(아래 참조).
    #
    #  ★★결정성 확보 후 총량 곡선(2026-08-07). 시드 주입 + workers=0 + GTC embedding 동결.
    #   같은 레시피 안의 총량 축이라 서로 비교가 성립한다.
    #    배수  앵커     판정     잃음   되살림   순증      정확도   run
    #     12   2,004   26/26 ✓   889   7,385   +6,496   82.07%  260807_1022
    #     48   8,016   26/26 ✓   548   8,278   +7,730   84.79%  260807_1302  ★정점 = 현행 확정값
    #     96  16,032   26/26 ✓   631   8,341   +7,710   84.74%  260807_1440
    #   - 판정 26/26 은 96배에서도 유지 = 상한선은 <타깃 매몰>이 아니라 <손실 반등>이 정했다.
    #   - 96배가 앞서는 지표는 되살림(+63) 하나뿐이고 잃음은 +83 악화 → 48배 채택.
    #   - 옛 12배(593)와 결정론 12배(889)를 직접 비교하지 말 것. embedding 동결로
    #     레시피 자체가 달라졌다. 비교는 같은 레시피 안(위 3행)에서만 성립한다.
    ...
    # ★★앵커 총량 확정(2026-08-10, 사용자 결정) = <절대값 8,016>.
    #  배수(--anchor-ratio)가 아니라 절대값을 쓰는 이유:
    #   ① 정점은 총량 8,016 에서 측정됐다(48배 = 167 × 48). 96배(16,032)는 더 나빴다.
    #   ② 2단계는 타깃이 167 → 약 322 크롭으로 늘어난다. 배수 48 을 유지하면 앵커가
    #      자동으로 약 15,450 = <실측에서 더 나빴던 96배 지점> 으로 올라간다.
    #   ③ 8,016(8,000 아님)을 쓰면 260807_1302 와 앵커 총량이 완전히 같아, 그 run 을
    #      A/A 재현 대조군으로 그대로 쓸 수 있다.
    #  타깃 크기가 크게 달라지는 회차에서 이 값이 여전히 정점인지는 미측정 — 재확인 대상.
    #  ▼▼ 2026-08-10: 비율 축을 닫고 <글자 겨냥> 으로 전환한다.
    #   비율(item/NUM/REST)은 방어력을 못 재는 잣대였다 - '로수반정10mg' 은 '영문+숫자'
    #   버킷이지만 한글을 5자 담는다. 548 손실의 한글 오독은 롱테일(최다 혼동쌍 8건)이라
    #   중요한 건 층 비율이 아니라 <깨진 그 글자를 앵커가 몇 번 보여줬나> 다.
    #   ★현행 8,016 앵커 실측: 깨진 한글 105종 중 <14종이 노출 0회>, 42종이 10회 미만.
    #    앵커를 늘려도 안 줄어든 게 아니라, 그 글자가 애초에 앵커에 없었다.
    #   ★후보 풀도 병목이었다: 정답풀 3.09M 중 컬럼 메타가 붙은 건 264,984 뿐이고
    #    itemName 으로 확인되는 건 35,507 장. 나머지 2.83M 은 컬럼 미상이라 통째로
    #    빠져 있었다. 겨냥은 컬럼이 아니라 글자를 보므로 한글 보유 크롭 728,162 전체를
    #    후보로 쓴다(--anchor-aim-pool korean).
    #   dry-run 실측(앵커 8,016 고정, 겨냥 60% / 무작위 품명 40% / 짧은숫자 800):
    #     지표                     현행      좁은풀(item)   ★넓은풀(korean)
    #     깨진한글 노출 0회        14종        4종            0종
    #     깨진한글 10회 미만       42종       26종            2종
    #     깨진한글 노출 중앙값     29회       58회           87회
    #     깨진영문 10회 미만        3종        1종            0종
    #     깨진영문 노출 중앙값     23회       96회          182회
    #     라벨 평균 길이          9.6자     12.4자         11.8자 (기준셋 12.2자)
    #     ㈜ 보유                  22장        3장          160장
    #   한글 총 노출량은 54,566 vs 53,849 로 거의 같다 - 같은 양을 롱테일에 고르게
