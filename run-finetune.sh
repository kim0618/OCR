...
PY
    ...
    #
    #  ▼ 현재 실험(v8): 거시 비율은 v5 그대로, 품명 내부 문자 조합만 층화.
    #   v5 실측 분해(중복 없음): 품명·비숫자 1,189 / 품명·짧은숫자 15 /
    #                            타컬럼·짧은숫자 498 / 타컬럼·나머지 302 = 2,004
    #   품명 1,189 배분 = √(평가 모수 × v5 잃어버림) 가중. 오류량만 따라가면 과추종이라
    #   모수를 함께 반영하는 완만한 위험 보정을 쓴다.
    #     조합   모수    v5잃음  →  장수
    #     HENS   9,980    248      446
    #     HEN    8,172    120      280
    #     H      6,212     74      192
    ...
    #     HES      312      9       15
    #     HE        81     11        9
    #   기각 규칙: 이 구성이 593 이하를 못 만들면 품명 내부 층화도 기각하고 v5 최종 유지.
    #  ★plan 은 절대 장수라 타깃 수가 바뀌면(2단계 이상) 총량이 안 맞아 빌드가 멈춘다.
    #   그때는 새 타깃 수 × 12 로 다시 배분해야 한다(멈추는 게 맞다 - 조용히 어긋나면
    #   비교가 깨진다).
    #  ★판정이 26/26 아래로 떨어지면 앵커가 타깃을 묻은 것 = 그 직전 배수가 상한선.
    DEMO_ANCHOR_RATIO=12.0       # 앵커 총량 = 타깃 크롭 × 이 배수 (167 × 12 = 2,004)
    DEMO_ANCHOR_PLAN="HENS=446,HEN=280,H=192,HS=111,HNS=87,HN=49,HES=15,HE=9,ITEMNUM=15,NUM=498,REST=302"
    DEMO_ANCHOR_ARGS="--anchor-ratio $DEMO_ANCHOR_RATIO --anchor-plan $DEMO_ANCHOR_PLAN"
