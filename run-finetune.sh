...
PY
    ...
    #  실측 이력(타깃=세파록스캡슐 167장 고정, base 대비 · 공백/표테두리 차이는 손실로 안 셈):
    #    배수  앵커   품명앵커  판정    잃어버림   순증
    #     0      0      0      22/26 ✗    -        -      주변 글자가 삽입·삭제·치환으로 붕괴
    #     3    501     40      26/26 ✓  6,066    -1,807   앵커 무작위(품명이 8%뿐)
    #     3    501    302      26/26 ✓  2,574    +3,060   ★구성만 품명 위주로 → 58% 감소
    #     6   1002    601      26/26 ✓  1,418    +5,231   ★총량 2배 → 다시 45% 감소
    #    12   2004  ~1200        ?        ?         ?      다음
    #  잃어버림 1,418 의 정체: 한글자 삭제 50% / 한글자 치환 42%, 타깃 글자 관련은 0.7%.
    #  = 데이터 모순(이론적 바닥)이 아니라 부수 피해라 더 줄일 여지가 있다.
    #  ★판정이 26/26 아래로 떨어지면 앵커가 타깃을 묻은 것 = 그 직전 배수가 상한선.
    DEMO_ANCHOR_ARGS="--anchor-ratio ${DEMO_ANCHOR_RATIO:-12.0}"
