...
PY
    ...
    #    12   2004   1603        ?        ?       ?        ?        ?     ← 지금 이 실행(품명 0.6→0.8)
    #  공정한 곡선은 균형 앵커가 적용된 3배(260804_1439)→6배→12배 셋이다.
    #  12배 실제 잃음 593 의 정체: 편집거리 1 단일 글자 오류가 92%.
    #  ★총량 축은 12배에서 끝났다(2026-08-05). 24배는 순증만 +232 이고 잃어버림이 반등해,
    #   사전 규칙("24배가 12배보다 나쁘면 12.0 으로 되돌린다")대로 12배로 복귀했다. 48배는 없다.
    #  ★다음 축은 총량이 아니라 <앵커 구성>이다. 판정셋 45,356 전수 대조 실측(2026-08-05):
    #   앵커 2,004 중 품명은 1,204(60%)뿐이고 나머지 402(20%)는 타 컬럼이라 품명 판정에
    #   기여가 0 이다. 품명만 판정하는 데모에서는 그 402 가 놀고 있다 → 품명 0.6→0.8.
    #   (짧은숫자 0.2 는 그대로 둔다. 줄이면 숫자 오독 61건이 되돌아와 변수가 둘이 된다)
    #  ★판정이 26/26 아래로 떨어지면 앵커가 타깃을 묻은 것 = 그 직전 배수가 상한선.
    DEMO_ANCHOR_RATIO=12.0       # 앵커 총량 = 타깃 크롭 × 이 배수
    DEMO_ANCHOR_ITEM=0.80        # 앵커 중 품명 비중. 0 = 예전 무작위 동작(A/B 용)
    DEMO_ANCHOR_SHORTNUM=0.20    # 앵커 중 짧은숫자(1~3자리) 비중. 숫자 붕괴 방어
