...
PY
    ...
    #    12   2004   1204      26/26 ✓    593   7,608   +7,015   83.21%  ★현재 최선
    #    24   4008  ~2405        ?        ?       ?        ?        ?     ← 지금 이 실행
    #  공정한 곡선은 균형 앵커가 적용된 3배(260804_1439)→6배→12배 셋이다.
    #  12배 실제 잃음 593 의 정체: 편집거리 1 단일 글자 오류가 92%.
    #  ★24배는 1차 1단계의 <마지막> 앵커 비교다. 결과와 무관하게 48배는 하지 않는다.
    #   24배가 12배보다 나쁘면 이 기본값을 12.0 으로 되돌리고 12배 모델을 부모로 쓴다.
    #  ★판정이 26/26 아래로 떨어지면 앵커가 타깃을 묻은 것 = 그 직전 배수가 상한선.
    DEMO_ANCHOR_ARGS="--anchor-ratio ${DEMO_ANCHOR_RATIO:-24.0}"
