...
def source_name(path: str) -> str:
    ...
    # run 폴더의 파생 이미지(processed/ · rotated/)는 sourceFile 이름에 .jpg 를 한 번 더 붙여 저장돼
    # 있다. 그걸 먹일 때는 꼬리를 떼야 compare_run 이 같은 GT·같은 Base 문서를 찾는다.
...
def main() -> int:
        ...
        try:
            if canned is not None:
                ...
                try:
                    ...
                    raw, ms2, meta = call_vlm(args.server, args.model, system, user, path,
                                              args.timeout, args.max_tokens,
    ...
    # 동시 수는 KV 캐시가 정한다 - 넘치면 vLLM 이 큐에 세우므로 틀려도 안전하다.
    if args.concurrency > 1 and canned is None:
