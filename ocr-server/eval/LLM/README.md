...
⚠️ `~/OCR/*.sh` 는 **레포 루트가 곧 그 자리**다(`OCR/run-vlm-*.sh` · git 추적 중). 2026-09-17 에 이걸 모르고 `eval/LLM/aws/` 로 복사해 사본을 만들었다가 되돌렸다 - 고칠 때는 **레포 루트에서 고치고 AWS 는 git pull** 이다. `aws/` 에는 run 별 스크립트(`run-vlm-500p.sh` · `run-vlm-500r.sh`)만 둔다.
