"""
T-10-final-rerun invoice_statement Template/RunOCR E2E verification.
...
OUT_JSON = REPORT_DIR / "T10_final_rerun_template_runocr_e2e_invoice_statement_20260515.json"
OUT_MD = REPORT_DIR / "T10_final_rerun_template_runocr_e2e_invoice_statement_20260515.md"
...
def select_template(sample: str, templates: list[dict[str, Any]]) -> dict[str, Any] | None:
    candidates = [row for row in templates if filename_matches(row, sample)]
    if not candidates:
        return None
    ...
    for index, row in enumerate(candidates):
        ...
        name_ok = 1 if filename_matches(row, sample) else 0
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append("# T-10-final-rerun invoice_statement Template/RunOCR E2E 최종 재검증 결과")
    lines.append("")
