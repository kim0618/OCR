"""
T-9-extend: invoice_statement Template/RunOCR saved-annotation E2E verification.
...
OUT_JSON = REPORT_DIR / "T9_extend_template_runocr_e2e_invoice_statement_20260514.json"
OUT_MD = REPORT_DIR / "T9_extend_template_runocr_e2e_invoice_statement_20260514.md"
...
def template_summary(template: dict[str, Any], manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    filename = (tj.get("file") or {}).get("name")
...
def call_extract(sample: str, template: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    with sample_path.open("rb") as f:
        files = {"file": (sample, f.read(), SAMPLE_MIMES[sample])}
    started = time.time()
    try:
        response = requests.post(f"{BASE_URL}/ocr/extract", data=data, files=files, timeout=360)
...
def verify() -> dict[str, Any]:
    ...
    for sample in PRIORITY_SAMPLES:
        ...
        if not matched:
            ...
            issues.append({
                ...
                "followup": "UI에서 table region/column guide 저장 후 T-9-extend 재실행",
            })
        ...
        for mode, meta in sample_report["modes"].items():
            if meta.get("doc_type") != "invoice_statement":
                issues.append({
                    ...
                    "followup": "documentType 라우팅 재확인(T-9-fix2 후보)",
                })
        samples[sample] = sample_report
    ...
    if not executed and missing_annotations:
        decision = "저장 annotation 부족 → UI에서 2~7.pdf template annotation 저장 후 재검증"
    elif missing_annotations:
        decision = "저장 annotation 부족 → UI에서 2~7.pdf template annotation 저장 후 재검증"
    elif len(exact) == len(executed) == len(SAMPLES):
        decision = "E2E 정상 → 거래명세서 Template/RunOCR 1차 완료"
    elif any("doc_type" in i.get("issue", "") for i in issues):
        decision = "documentType 라우팅 문제 재발 → T-9-fix2"
    elif issues:
        decision = "tableBounds/columnGuides 좌표 문제 → T-9-colguides-fix"
    else:
        decision = "다음 문서 유형 확장 가능"
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# T-9-extend Template/RunOCR E2E invoice_statement 확장 검증 결과")
    lines.append("")
    ...
    lines.append(f"- 실제 저장 template 사용 여부: `{report['method']['actualSavedTemplateUsed']}`")
    lines.append(f"- API 직접 호출 여부: `{report['method']['apiDirectCall']}`")
    lines.append(f"- 사용한 payload: {report['method']['payload']}")
    lines.append(f"- 한계: {report['method']['limits']}")
    lines.append("")
    lines.append("## 3. Template annotation 확인")
    lines.append("| 샘플 | template 존재 | template_id | documentType | table region | colGuides | 판정 |")
    lines.append("|---|---|---|---|---|---|---|")
    for sample in SAMPLES:
        ...
        lines.append(
            ...
            f"{md(tmpl.get('judgement') or row.get('annotationStatus'))} |"
        )
    lines.append("")
    lines.append("## 4. RunOCR payload 확인")
    lines.append("| 샘플 | 실행 여부 | regions 전달 | documentType source | tableBounds 유도 | columnGuides 유도 |")
    lines.append("|---|---|---|---|---|---|")
    for sample in SAMPLES:
        row = samples.get(sample) or {}
        tmpl = row.get("template") or {}
        mode_a = (row.get("modes") or {}).get("A") or {}
        mode_b = (row.get("modes") or {}).get("B") or {}
        doc_source = "template metadata + explicit payload" if row.get("apiExecuted") else "-"
        lines.append(
            f"| {sample} | {md(row.get('apiExecuted'))} | {md((mode_b.get('payload') or {}).get('regions'))} | "
            f"{doc_source} | {md(bool(tmpl.get('table_region')) if tmpl else None)} | "
            f"{md((tmpl.get('colGuides_count') or 0) > 0 if tmpl else None)} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("## 9. annotation 미존재 샘플 후속")
    lines.append("| 샘플 | 필요한 작업 |")
    lines.append("|---|---|")
    if report["missingAnnotations"]:
        for item in report["missingAnnotations"]:
            lines.append(f"| {md(item.get('sample'))} | {md(item.get('needed'))} |")
    else:
        lines.append("| 없음 | - |")
    lines.append("")
