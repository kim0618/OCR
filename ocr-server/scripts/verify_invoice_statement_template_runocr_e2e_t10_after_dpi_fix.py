"""
T-10 after DPI-fix: invoice_statement Template/RunOCR E2E verification.
...
OUT_JSON = REPORT_DIR / "T10_after_dpi_fix_template_runocr_e2e_invoice_statement_20260515.json"
OUT_MD = REPORT_DIR / "T10_after_dpi_fix_template_runocr_e2e_invoice_statement_20260515.md"
...
def _get_table_expected_columns(sample: str, manifest: dict[str, Any]) -> dict[str, Any] | None:
    for item in manifest.get("items", []):
        if item.get("filename") == sample:
...
def call_extract(sample: str, template: dict[str, Any], mode: str, manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    tj = template.get("template_json") or {}
    regions = tj.get("regions") or []
    data = {
    ...
    # template metadata routing (backend derives _tec from manifest via T-10-fix-colguides).
    # Mode B passes saved regions like RunOCR UI and includes tableExpectedColumns.
    if mode == "B":
        data["regions"] = json.dumps(regions, ensure_ascii=False)
        if manifest:
            _tec = _get_table_expected_columns(sample, manifest)
            if _tec:
                data["tableExpectedColumns"] = json.dumps(_tec, ensure_ascii=False)
...
def verify() -> dict[str, Any]:
    ...
    for sample in PRIORITY_SAMPLES:
        ...
        for mode in ("A", "B"):
            api_result = call_extract(sample, matched["raw"], mode, manifest=manifest)
    ...
    executed_by_sample = {row.get("sample"): row for row in executed}
    if not executed and missing_annotations:
        decision = "documentType 누락 → 저장 payload 재확인"
    elif missing_annotations:
        if all(
            executed_by_sample.get(sample, {}).get("rowCountStatus") == "exact"
            for sample in ["5.pdf", "2.pdf"]
        ):
            decision = "5.pdf/2.pdf E2E 정상 → 7.pdf/6.pdf/3.pdf/4.pdf annotation 저장 후 전체 재검증"
        else:
            decision = "tableBounds 좌표 문제 → UI annotation 조정 후 재저장"
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# T-10 after DPI-fix Template/RunOCR E2E 재검증 결과")
    lines.append("")
    ...
    for sample in ["5.pdf", "2.pdf", "7.pdf", "6.pdf", "3.pdf", "4.pdf", "1.jpg"]:
        ...
        lines.append(f"- fieldCounts: {md(mode.get('rowFieldCounts'))}")
