"""
T-10 after 6pdf invoice_statement Template/RunOCR E2E final verification.
...
OUT_JSON = REPORT_DIR / "T10_after_6pdf_template_runocr_e2e_20260515.json"
OUT_MD = REPORT_DIR / "T10_after_6pdf_template_runocr_e2e_20260515.md"
...
def is_target_template(row: dict[str, Any], sample: str) -> bool:
    ...
    if sample == "4.pdf":
        names = " ".join(all_name_fields(row)).lower()
        return "거래_4" in names
    if sample == "6.pdf":
        names = " ".join(all_name_fields(row)).lower()
        return "거래_6" in names
    if sample == "7.pdf":
        names = " ".join(all_name_fields(row)).lower()
...
def sample_specific_checks(sample: str, result: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if sample == "6.pdf":
        ...
        checks["quantity 0 row kept"] = '"quantity": "0"' in json.dumps(rows, ensure_ascii=False) or " 0 " in f" {text} "
    if sample == "7.pdf":
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append("# T-10 after 6pdf Template/RunOCR E2E 최종 검증 결과")
    lines.append("")
